...
public class CharCompactStack implements Stack<Character> {
    ...
    public CharCompactStack() {
        this(32);
    }

    public CharCompactStack(int initialCapacity) {
        this.internalMaxSize = (initialCapacity >> 2) + 1;
        container = new long[internalMaxSize];
    }

    @Override
    public void push(Character el) throws IndexOutOfBoundsException {
    ...
    @Override
    public Character pop() throws IndexOutOfBoundsException{
        if (currentSize == 0) {
            throw new IndexOutOfBoundsException("Stack is empty");
        }
        long box = container[(currentSize - 1) >> 2];
        return (char)(ZERO | (box >> (OFFSET_TABLE[(((currentSize-- - 1) & 3))])));
    }
